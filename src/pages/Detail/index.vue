            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active:spuSaleAttrValue.isChecked==1}"
                    v-for="(spuSaleAttrValue,imdex) in spuSaleAttr.spuSaleAttrValueList"
...
export default {
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      //遍历全部的售卖的属性值isChecked=0，即所有售卖属性值没有高亮
      arr.forEach(item => {
        item.isChecked = 0;
      });
      //点击的那个售卖属性值有高亮
    ...
    async addShopcar() {
      ...
      try {
        ...
        //产品信息的数据【比较复杂:skuInfo】，通过会话存储（不会持久化，会话结束数据消失）
        //本地存储|会话存储，一般存的都是字符串
